    ...
    try:
        for state in gen:
            ...
            if stype == "plan":
                ...
                title = state["title"]
                if agent == -1:
                    # CBS constraint-tree node. Always sync paths even on
                    # skipped frames so the renderer is never stale when
                    # a frame does fire.
                    if "paths" in state:
                    ...
                    if steps % skip_frames != 0:
                        continue
                    # Build a progress bar — without it CBS looks completely
                    # frozen during its planning phase which can take hundreds
                    # of iterations.
                    _max_nodes = state.get("max_nodes", _CBS_MAX_NODES_FALLBACK)
                    _pct       = min(100, int(steps / max(1, _max_nodes) * 100))
                    _filled    = _pct // 5
                    _bar       = "█" * _filled + "░" * (PROGRESS_BAR_WIDTH - _filled)
                    hud = (
                        f"{C_BIGO}CBS — PLANNING PHASE{C_END}"
                        f"  [{_bar}] {_pct}%\n"
                        f"   {title}\n"
                        f"   {C_DIM}Agents are offline — CBS finds a conflict-free"
                        f" plan before anyone moves.{C_END}"
                    )
                else:
                    hud = (
            ...
            # ── Simulation step ─────────────────────────────────────────────
                ...
                # First tick after CBS planning — flash a transition banner so
                # the user knows the agents are now actually moving.
            ...
            # ── Done ───────────────────────────────────────────────────────
                ...
                message: str       = state.get("message", "")
                # Pass paths=None on the final frame — current_paths contains
                # the full simulation history which renders as a messy trail.
                render_mapf(
                    maze,
                    positions=current_positions,
                    goals=goals,
