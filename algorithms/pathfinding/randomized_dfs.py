"""
algorithms/pathfinding/randomized_dfs.py — Randomized DFS.

Standard DFS but DIRECTIONS are shuffled at every expansion. Keeps all the
correctness guarantees of DFS (finds a path if one exists, O(V) space) but
produces a different, jagged path on every run. Good for demonstrating how
randomness can be injected into a deterministic algorithm.
"""
...
def solve(
    ...
    while stack:
        ...
        random.shuffle(dirs)  # this one line is what makes it "randomized"
        ...
        yield {"type": "step", "r": r, "c": c, "steps": steps,
               "title": "Randomized DFS (Drunk)", "restore": ".", "pq_info": ""}
